...
import { orcText } from "./orchelper"

export type Tool = "多边形" | "矩形" | "圆形" | "选择" | "删除" | "测试";
export type Image = {
    data: p5.Image | null,
    file: p5.File | null,
};
...
export class imageMapCreator {
    ...
    protected menu = {
        ...
        DeleteArea: {
            onSelect: (target: Element, key: any, item: HTMLElement, area: Area) => { this.deleteArea(area); },
            label: "删除区域",
        },
        OcrText: {
            onSelect: (target: Element, key: any, item: HTMLElement, area: Area) => { this.OcrText(area); },
            label: "识别文字",
        },
        MoveFront: {
    ...
    private setup(): void {
        ...
        this.settings = QuickSettings.create(this.p5.width + 5, 0, "图片地图创建器", this.p5.canvas.parentElement)
            .setDraggable(false)
            .addText("地图名称", "", (v: string) => { this.map.setName(v) })
            .addDropDown("工具", ["多边形", "矩形", "圆形", "选择", "删除", "测试"], (v: ToolLabel) => { this.setTool(v.value) })
            .addBoolean("默认区域", this.map.hasDefaultArea, (v: boolean) => { this.setDefaultArea(v) })
            .addButton("撤销", this.undoManager.undo)
            .addButton("重做", this.undoManager.redo)
            .addButton("清除", this.clearAreas.bind(this))
            .addButton("生成HTML", () => { this.settings.setValue("输出", this.map.toHtml()) })
            .addButton("生成SVG", () => { this.settings.setValue("输出", this.map.toSvg()) })
            .addTextArea("输出")
            .addButton("保存", this.save.bind(this));
        ...
        document.getElementById("输出").setAttribute("onFocus", "this.select();");
    }
    ...
    handeFile(file: p5.File): void {
        if (file.type == "image") {
            ...
            if (!this.map.getName()) {
                ...
                this.settings.setValue("地图名称", this.map.getName());
            }
    ...
    setOutput(): void {
        switch (this.tool) {
            ...
            case "选择":
                if (this.mouseIsHoverSketch()) {
                    let title = this.hoveredArea ? this.hoveredArea.getTitle() : "none";
                    this.settings.setValue("输出", title);
                }
                break;
        }
    }
    ...
    setTitle(area: Area | null): void {
        if (this.tool == "测试" && area && area.getTitle()) {
            //@ts-ignore p5 types does not specify the canvas attribute
    ...
    importMap(json: string): void {
        ...
        this.settings.setValue("地图名称", objectMap.name);
        this.settings.setValue("默认区域", objectMap.hasDefaultArea);
        this.reset();
    }
    ...
    deleteArea(area: Area): void {
        let id = area.id;
        if (id === 0) {
            this.settings.setValue("默认区域", false);
        } else {
    ...
    async OcrText(area: Area) {
        this.settings.setValue("输出", "开始识别，请稍后...");
        var minPoint = area.copyCoords()[0];
        var maxPoint = area.copyCoords()[0];
        for (let index = 0; index < area.copyCoords().length; index++) {
            const tempPoint = area.copyCoords()[index];
            minPoint.x = Math.min(minPoint.x, tempPoint.x);
            minPoint.y = Math.min(minPoint.y, tempPoint.y);
            maxPoint.x = Math.max(maxPoint.x, tempPoint.x);
            maxPoint.y = Math.max(maxPoint.y, tempPoint.y);
        }
        var top = minPoint.y
        var left = minPoint.x
        var width = maxPoint.x - minPoint.x
        var height = maxPoint.y - minPoint.y
        var text = await orcText(this.img.file, { top, left, width, height });
        this.settings.setValue("输出", text.split(" ").join(""));
    }
    ...
    setAreaUrl(area: Area): void {
        ...
        let input = prompt("请输入选中区域的地址", href ? href : "http://");
        if (input) {
            area.setHref(input);
            this.undoManager.add({
    ...
    setAreaTitle(area: Area): void {
        ...
        let input = prompt("请输入选中区域的标题", title ? title : "");
        if (input) {
            area.setTitle(input);
            this.undoManager.add({
    ...
    setDefaultArea(bool: boolean): void {
        ...
        this.undoManager.add({
            undo: () => {
                ...
                this.settings.setValue("默认区域", !bool)
            },
            redo: () => {
                this.map.setDefaultArea(bool); // semble redondant
                this.settings.setValue("默认区域", bool)
